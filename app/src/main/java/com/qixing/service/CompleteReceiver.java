...
//                Uri downloadFileUri = downloadManager.getUriForDownloadedFile(id);
                ...
                } else if (id == Long.parseLong(MyApplication.getInstance().getVideoId())) {
                    Intent install = new Intent(Intent.ACTION_VIEW);
//                Uri downloadFileUri = downloadManager.getUriForDownloadedFile(id);
                    String uriString = getFilePathFromUri(context, Uri.parse(path));//TODO 转换path路径 否则报解析包错误
                    System.out.println("===========================CompleteReceiver 转换后----路径uriString = " + uriString);
                    ...
                    install.setDataAndType(Uri.fromFile(new File(uriString)), "video/*");
                    context.startActivity(install);
                }else if(id==Long.parseLong(MyApplication.getInstance().getPdfId())){
                    Intent install = new Intent(Intent.ACTION_VIEW);
//                Uri downloadFileUri = downloadManager.getUriForDownloadedFile(id);
                    String uriString = getFilePathFromUri(context, Uri.parse(path));//TODO 转换path路径 否则报解析包错误
                    System.out.println("===========================CompleteReceiver 转换后----路径uriString = " + uriString);
                    install.addCategory(Intent.CATEGORY_DEFAULT);
                    install.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    install.setDataAndType(Uri.fromFile(new File(uriString)), "application/pdf");
                    context.startActivity(install);
                }
