...
public class SubFragment1 extends BaseFragment implements View.OnClickListener {
	TextView msg;
...
//				AppManager.getAppManager().startNextActivity(getActivity(), MyWebActivity.class, intent);
				isnosee("3",banners.get(position).getId(),Constant.URL_INDEX_SELZX + "/id/" + banners.get(position).getId());//类型（1直播  2视频  3资讯  4干货 ）
			}
		});
...
//				Intent intent = new Intent();
//				intent.putExtra("url", Constant.BASE_URL + Constant.URL_INDEX_SELZX + "/id/" + list.get(position).getId());
//				intent.putExtra("title", list.get(position).getTitle());
//				AppManager.getAppManager().startNextActivity(getActivity(), MyWebActivity.class, intent);
				infos.add(position);
...
//				}
				isnosee("3",list.get(position).getId(),Constant.URL_INDEX_SELZX + "/id/" + list.get(position).getId());//类型（1直播  2视频  3资讯  4干货 ）
//				list.get(position).setState("1");
//				mF4RecommendedInfoAdapter.notifyDataSetChanged();
			}
		});
//		initTestInfo();
...
//			mProgressDialog.show();
		...
		System.out.println("===========================咨询列表 url ======= " + url);
				...
				if (!TextUtils.isEmpty(response.toString())) {
					...
					if (mInfoJson.getResult().equals("1")) {
						...
						if (!(mInfoJson.getList() == null || mInfoJson.getList().size() == 0 ||  "".equals(mInfoJson.getList()))) {
							...
							mF4RecommendedInfoAdapter = new F4RecommendedInfoAdapter(getActivity(),list);
							lv_info.setAdapter(mF4RecommendedInfoAdapter);
						}

//						for (int i = 0; i <infos.size() ; i++) {
//							list.get(infos.get(i)).setState("1");
//						}
//						mF4RecommendedInfoAdapter.notifyDataSetChanged();
					} else {
//						Toasts.show(mShoppingMineBean.getMessage());
	...
	private void initMoreDatas(){
		...
		System.out.println("===========================咨询列表 更多 url ==== " + url);
